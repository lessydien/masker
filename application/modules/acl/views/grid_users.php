			...
			<div class="modal-body">
				...
				<div class="row">
					...
					<div class="col-md-6" >
						...
						<div class="form-group" id="cgolongan">
      						<label>Golongan</label>
							<select id="golongan" name="golongan" class="form-control input-sm">
								<option value="">-- Pilih --</option>
								<?php foreach($ls_golongan as $golongan):?>
								<option value="<?php echo $golongan->id_golongan;?>"><?php echo $golongan->pangkat;?></option>
								<?php endforeach;?>
							</select>
      					</div>
					</div>
  ...
  function responseHandler(res) {
	...
	$(document).ready(function(){		
		...
		$('#btn-edit').click(function(e){
			...
			$('#golongan').val(row[0].golongan_id);
				...
				if (r == true) {
					...
					$.ajax({
						...
						success: function(data){
							if(data.resp){
								// alert("Selamat,\n\r"+data.message);
								$('#msg-alert').html('<div class="alert alert-info alert-dismissible">'+
								  						'<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>'+
														'<h4><i class="icon fa fa-check"></i> Selamat!</h4>' + data.message +
													'</div');
		...
		$('#frm-org-mdl').submit(function(e){
			...
			$.ajax({
					...
					success: function(data){
						...
						if(data.resp){
							// alert("Selamat,\n\r"+data.message);
							$('#alert-msg').html('<div class="alert alert-success alert-dismissible">'+
													'<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>'+
													'<h4><i class="icon fa fa-check"></i> Selamat!</h4>' + data.message +
												'</div');
							$('#mdl_org').modal('hide');
